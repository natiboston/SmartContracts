...
contract contractPuja  is Ownable, ErrorLogger, contractPost, contractAdvertiser, MngWallet {
	...
	function requestAdvPuja(address _owneraddv, uint256 _idAd, address _ownerpost, 
        ...
        if( ( wallets[_owneraddv].balance  <  int256(_valuepuja) ) || 
	...
	function acceptAdvPuja (address _owneraddv, uint256 _idAd, address _ownerpost,  
		uint16 _year, uint8 _month, uint8 _day, uint256 _valuepuja ) onlyOwnerAd(_owneraddv) returns (bool result){
        ...
        if (adver_oldpuja !=address(0x0)){
            ...
            wallets[_owneraddv].balance += int256(value_oldpuja);
		...
		wallets[_owneraddv].balance -= int256(_valuepuja);
	...
	function ClosePujaDay(address _ownerpost, uint16 _year, uint8 _month, uint8 _day) returns (bool result){
		PostePuja[_ownerpost][_year][_month][_day].closed = true;
		PostePuja[_ownerpost][_year][_month][_day].active = false;
		return PostePuja[_ownerpost][_year][_month][_day].closed;
	}
