...
library PostLibrary{
  ...
  function func_newPostPujaAllYear (PostSt storage is_post, uint in_year, 
                   uint in_inipuja, uint in_finpuja ) returns (bool){
  ...
  function func_UndoPuja( PostSt storage post, uint in_year, uint in_day,  uint in_valor) returns (uint vn_position, bool ob_result ) {
     uint i = 0;
     uint myday= in_year*1000 + in_day;
     uint vn_date = now;
  ...
  function func_Puja(AddSt storage is_add, PostSt storage post,
                    uint in_year, uint in_day,  uint in_valor) 
       returns ( uint on_result, uint on_numadds)  {
